        ...
        if (!err) {
          ...
          db.run(`
            ...
            if (!err) {
              ...
              db.run(`CREATE UNIQUE INDEX email_UIDX ON users (email)`, [], err => {
                if (!err) {
                  // create indexes for table 'users'
                  db.run(`CREATE UNIQUE INDEX phone_UIDX ON users (phone)`, [], err => {
                    if (!err) {
                      ...
                      db.run(`
                        ...
                        if (!err) {
                          callback('Database has been created. Try to send data another one time.');
                          return false;
                        } else {
                          callback('Cannot create table \'tokens\', ' + err.message);
                          return false;
                        }
                      });
                    } else {
