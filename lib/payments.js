...
payments._handlers.post = (data, callback) => {
  ...
  tokens.verifyToken(token, (userUid) => {
    if (userUid) {
      ...
      payments._customer[mode](userUid);
...
/**
 * Creates a customer in the Stripe portal if it does not exist there.
 * @param {number} userUid User database table related unique id.
 * Required data: none.
 * Optional data: none.
 * @param {function} callback The function runs after data is checked when all checks are passed.
 */
payments._customer.createCustomer = (userUid, callback) => {
  callback(400, {'Details': userUid});

};


/**
 * Makes payment request to the Stripe portal in attempt for paying given order.
 * @param {number} orderUid Order database table related unique id.
 * Required data: none.
 * Optional data: none.
 * @param {function} callback The function runs after data is checked when all checks are passed.
 */
payments._customer.payOrder = (orderUid, callback) => {
  callback(400, {'Details': orderUid});
