...
handlers._users.put = (data, callback) => {
  ...
  if (phone) {
    // SQL reuest - get the user from the database
    let sqlRequest = format(`
    SELECT
      uid
    FROM users
    ...
    _db.get(sqlRequest, (err, data) => {
      if (!err && data) {
        // SQL reuest - update user in the database
        sqlRequest = format(`
        UPDATE users SET
          first_name = '%s',
          last_name = '%s',
...
handlers._tokens.verifyToken = (token, phone, callback) => {
  ...
  SELECT
    ...
    t.expired_at
  FROM users u
    INNER JOIN tokens t ON t.user = u.uid
  WHERE 
    t.token = '%s'
  AND
    u.phone = '%s'
  ...
  _db.get(sqlRequest, (err, data) => {
    if (!err && data) {
      if (data.u.phone == phone && data.t.expires_at > Date.now()) {
        callback(true);
      } else {
        callback(false);
      }
    } else {
      callback(500, { 'Error': 'Could not verify the token', 'Details': err });
    }
  });
  // _db.read('tokens', id, (err, tokenData) => {
  //   if (!err && tokenData) {
